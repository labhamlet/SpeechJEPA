...
class JEPAQuantized(pl.LightningModule):
    """
    ...
    def __init__(
        ...
        normal_init_codebook: bool = False,
        ...
        **kwargs : dict[str, Any],
        ...
        #Init the codebook heads for each layer.
        self.heads = torch.nn.ModuleList([
                nn.Linear(
                    self.decoder_embedding_dim,
                    self.codebook_size,
                )
                for i in range(average_top_k_layers)
            ]
        )
        if self.normal_init_codebook:
            codebooks = torch.normal(0.0, (1 / self.codebook_size**0.5),
                        size=(average_top_k_layers, self.codebook_size, self.encoder_embedding_dim))
        else:
            codebooks = torch.randn(average_top_k_layers, self.encoder_embedding_dim, self.codebook_size)
            codebooks = F.instance_norm(codebooks).transpose(1, 2)

        self.register_buffer("codebooks", codebooks)
        self.register_buffer("codebook_cnts", torch.ones(average_top_k_layers, self.codebook_size))
    ...
    def training_step(self, batch: torch.Tensor, batch_idx: int) -> ForwardReturn:
        ...
        out, prep = self(student_input, teacher_input, ctx_masks, target_indices, ctx_and_target_masks, teacher_padding_mask)
    ...
    def _forward_teacher(self, x : torch.Tensor, padding_mask : torch.Tensor | None) -> torch.Tensor:
        layer_outputs =[]
        for i, bl in enumerate(self.teacher_encoder.layers): 
            x = bl(x, src_key_padding_mask=padding_mask)
        ...
        if self.hparams.average_top_k_layers > 1:
            ...
            return [targets[i] for i in range(targets.size(0))]
        else:
            return [layer_outputs[-1]]
    
    @staticmethod
    def _compute_ppl(y: torch.Tensor, input_onehot: bool, tokenwise: bool = False) -> torch.Tensor:
    ...
    def _quantize_target(self, codebook_index, target):
        codebook = self.codebooks[codebook_index].float() / self.codebook_cnts[codebook_index].unsqueeze(1)
    ...
    def _update_codebook(self, 
                         ...
                         onehot_target, 
                         pred):
        target_ppl = self._compute_ppl(onehot_target,input_onehot=True)
        pred_ppl = self._compute_ppl(pred.float(),input_onehot=False)
        if self.training and self.codebook_decay<1:
            ...
            self.codebook_cnts[codebook_index]  = alpha.squeeze(1) * self.codebook_cnts[codebook_index] + (1-alpha).squeeze(1) * count
            self.codebooks[codebook_index] = alpha * self.codebooks[codebook_index] + (1-alpha) * memory
        return target_ppl, pred_ppl
    

    def codebook_loss(self, pred, targets, target_indices):
        # target_indices should be boolean. Shape: (B*N, S)
        active_mask = target_indices.bool()
        num_active_targets = active_mask.sum()
        
        _acc_loss = 0.0
        total_target_ppl = 0.0

        for i, target in enumerate(targets):
            flat_target = target[active_mask]
            
            head_pred = self.heads[i](pred).float()
            flat_pred = head_pred[active_mask]
            
            onehot_target = self._quantize_target(codebook_index=i, target=flat_target)
            
            log_probs = F.log_softmax(flat_pred, dim=-1)
            loss = torch.sum(-onehot_target * log_probs, dim=-1).sum()
            
            _acc_loss = _acc_loss + loss
            
            t_ppl, p_ppl = self._update_codebook(
                codebook_index=i,
                target=flat_target,
                onehot_target=onehot_target,
                pred=log_probs
            )
            total_target_ppl += t_ppl
    
        final_loss = (_acc_loss / len(targets)) / num_active_targets
        avg_target_ppl = total_target_ppl / len(targets)
        return final_loss, avg_target_ppl
    ...
    def forward(self, 
            ...
            teacher_features = self._extract_audio(teacher_input, mask_indices=None, padding_mask=teacher_padding_masks)
            teacher_features = teacher_features.detach()
            targets = self._forward_teacher(teacher_features, padding_mask=teacher_padding_masks)
            
            nr_targets = target_indices.shape[1]
            aligned_targets =[repeat(t, 'B S E -> (B N) S E', N=nr_targets) for t in targets]

            if target_indices.ndim == 3:
                target_indices = rearrange(target_indices, 'B N S -> (B N) S')

            local_features = self._extract_audio(student_input, mask_indices=ctx_masks, padding_mask=teacher_padding_masks)
            ...
            preds = self.decoder_forward(contextual_features, ctx_masks, nr_targets=nr_targets, src_key_padding_mask=ctx_and_target_masks)
            
            loss, prep = self.codebook_loss(preds, aligned_targets, target_indices)
            
            return ForwardReturn(loss=loss), prep
    ...
    def decoder_forward(self, contextual_features, ctx_mask, nr_targets, src_key_padding_mask=None):
        ...
        #Decoder maps directly to codebooks via self.heads!
        return tgt
