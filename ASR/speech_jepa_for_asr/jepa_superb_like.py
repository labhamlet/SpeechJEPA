...
class SpeechJEPAForCTC(pl.LightningModule):
    def __init__(
        ...
        self.post_extraction_mapper = getattr(pretrained_jepa, "post_extraction_mapper", None)
        self.local_feature_norms = pretrained_jepa.local_feature_norms
        self.encoder = pretrained_jepa.encoder
        self.encoder_pos_emb = getattr(pretrained_jepa, "encoder_pos_emb", None)
        self.encoder_sin_cos_emb = getattr(pretrained_jepa, "encoder_sin_cos_pos_embedding", None)
        self.decoder_sin_cos_emb = getattr(pretrained_jepa, "decoder_sin_cos_pos_embedding", None)

        if with_decoder:
            self.encoder_to_decoder_mapper = getattr(pretrained_jepa, "encoder_to_decoder_mapper", None)
            self.decoder_to_encoder_mapper = getattr(pretrained_jepa, "decoder_to_encoder_mapper", None) # Removed trailing space
            self.decoder = pretrained_jepa.decoder 
            self.decoder_pos_embed =  getattr(pretrained_jepa, "decoder_pos_emb", None)
        ...
        if self.hparams.freeze_encoder_updates > 0:
            ...
            if self.encoder_pos_emb:
                self.encoder_pos_emb.requires_grad_(False)
            if self.post_extraction_mapper:
                self.post_extraction_mapper.requires_grad_(False)
            if with_decoder:
                if self.encoder_to_decoder_mapper:
                    self.encoder_to_decoder_mapper.requires_grad_(False)
                if self.decoder_to_encoder_mapper:
                    self.decoder_to_encoder_mapper.requires_grad_(False)
                self.decoder.requires_grad_(False)
                if getattr(self, "decoder_pos_embed", None):
                    self.decoder_pos_embed.requires_grad_(False)

        self.beam_search_dev = None
        self.beam_search_test = None

        nr_encoder_layers = len(self.encoder.layers) + 1  # Added +1 for the initial prepended state
        nr_decoder_layers = len(self.decoder.layers) if with_decoder else 0
        self.layer_weights = nn.Parameter(torch.ones(nr_decoder_layers + nr_encoder_layers))
    ...
    def run_through_model(self, 
        ...
        hidden, features = self.encoder(local_features, 
                                        src_key_padding_mask=attention_mask, 
                                        output_hidden_states = True)  

        return BaseModelOutput(
            last_hidden_state=hidden,
            hidden_states=[local_features] + features + [hidden],
        )
    ...
    def forward(self, audio: torch.Tensor, attention_mask: torch.Tensor, padding_mask: torch.Tensor):
        ...
        # Encoder Pos Embeddings
        if self.use_kernel_dropout_encoder:
            x = x + self.encoder_pos_emb(x, mask=~attention_mask)                
        elif self.encoder_pos_emb is not None:
            x = x + self.encoder_pos_emb(x)
        
        
        x = self.local_feature_norms(x)        
        
        outputs = self.run_through_model(x, attention_mask=~attention_mask)
        
        # Stack hidden states: (num_layers, B, T, D) -> (B, T, num_layers, D)
        stacked_hidden = torch.stack(outputs.hidden_states, dim=2)
        weights = torch.softmax(self.layer_weights, dim=0)  # (num_layers,)

        print(weights)
        weighted_hidden = (stacked_hidden * weights.view(1, 1, -1, 1)).sum(dim=2)
        return self.lm_head(self.dropout(weighted_hidden))
