...
from speech_jepa_for_asr.dinosr_ctc import DinoSRForCTC, load_dinosr
...
# DinoSR uses the identical wav2vec2-style conv front end (kernels
# [10,3,3,3,3,2,2], strides [5,2,2,2,2,2,2], 50 Hz), so the token/length
# function for the datamodule is unchanged.
conv_cfg = {
...
def train_librilight(
    ...
    model = DinoSRForCTC(
        pretrained_dinosr=pretrained_dinosr_model,
    ...
    for rank, (ckpt_path, val_wer) in enumerate(ranked, start=1):
        ...
        ckpt_model = DinoSRForCTC.load_from_checkpoint(
            ckpt_path,
            pretrained_dinosr=pretrained_dinosr_model,
            encoder_embedding_dim=encoder_embedding_dim,
            bundle=bundle,
...
def load_model(cfg):
    """
    The fairseq .ckpt already contains config + weights, so point `model_path`
    in the yaml at the downloaded checkpoint (dinosr.ckpt) - no other config
    changes needed. The architecture blocks in the yaml (transformer_*_cfg,
    pos_embedding, decoder, decoder_groups, layer_drop, attn/activation/hidden
    dropout) are simply unused here: the checkpoint defines its own
    architecture and regularization.

    SpecAugment: `mask_time_prob` is wired into the model's own time masking.
    `mask_feature_prob` is unused (channel masking is disabled in the loader;
    at 0.004 its effect is negligible anyway).
    """
    print(cfg.model_path)
    model, embed_dim = load_dinosr(
