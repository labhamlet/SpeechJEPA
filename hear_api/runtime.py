...
#         x = self.model.get_audio_representation(audio, padding_mask=None)
...
class RuntimeSpeechJEPA(torch.nn.Module):
    ...
    def get_timestamp_embeddings(self, audio):
        ...
        # embeddings =[]
        # # Fix: math.ceil ensures we do not skip the last segment
        # num_chunks = math.ceil(audio.shape[-1] / self.unit_frames)
        # #Normalize the full audio
        # audio = instance_normalize(audio)
        # for i in range(num_chunks):
        #     #Pass the chunks to the model.
        #     mt = audio[..., i * self.unit_frames : (i + 1) * self.unit_frames]
        #     embedding = self.model.get_audio_representation(mt, padding_mask=None)
